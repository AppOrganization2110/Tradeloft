    ...
    <section className="space-y-4">
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-700 bg-bg-secondary p-6 shadow-card sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">Alle gespeicherten Analysen und nachträgliche Ausstiegsdaten.</p>
        </div>
        <button
          type="button"
          onClick={onExport}
          className="inline-flex min-h-[44px] items-center justify-center rounded-3xl bg-accent-cyan px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-teal-400"
        >
          JSON exportieren
        </button>
      </div>
            ...
            <div key={trade.id} className="rounded-3xl border border-slate-700 bg-bg-tertiary p-6 shadow-card">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {Object.entries(trade.signals).map(([key, value]) => (
                  <div key={key} className="rounded-3xl border border-slate-700 bg-bg-secondary p-4 text-sm text-slate-300">
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <div className="rounded-3xl border border-slate-700 bg-bg-secondary p-4 text-sm text-slate-300">
                  <p className="text-slate-400">Exit-Preis</p>
                  <p className="mt-2 text-lg text-white">{trade.exitPrice ? `${trade.exitPrice.toFixed(2)} €` : 'offen'}</p>
                </div>
                <div className="rounded-3xl border border-slate-700 bg-bg-secondary p-4 text-sm text-slate-300">
                  <p className="text-slate-400">Ergebnis</p>
                  <p className="mt-2 text-lg text-white">{trade.result ? `${trade.result.toFixed(2)} €` : 'offen'}</p>
                </div>
                <div className="rounded-3xl border border-slate-700 bg-bg-secondary p-4 text-sm text-slate-300">
                  <p className="text-slate-400">Exit-Zeit</p>
                  <p className="mt-2 text-lg text-white">{trade.exitTimestamp ?? 'keine'}</p>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-3xl border border-slate-700 bg-bg-secondary p-4 text-sm text-slate-300">
                  ...
                  <textarea
                    value={trade.note ?? ''}
                    onChange={(event) => onUpdateTrade({ ...trade, note: event.target.value })}
                    className="mt-2 h-24 w-full resize-none rounded-2xl border border-slate-700 bg-bg-tertiary px-3 py-2 text-white outline-none"
                  />
                </div>
              </div>
            </div>
